...
**Bucket:** `kj-test-bucket2`

**Scenario Tested:**
- Public read access to `public_space/*` folder only (HTTPS enforced)
- User-specific permissions: `kishan-iam` (PutObject) vs `kishan-iam-2` (DeleteObject)
  ...
  "Statement": [
    {
      ...
      "Principal": "*",
      "Action": "s3:GetObject",
      "Resource": "arn:aws:s3:::kj-test-bucket2/public_space/*",
      "Condition": {
        "Bool": {
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::349853822779:user/kishan-iam"
      },
      "Action": [
        "s3:PutObject",
        "s3:GetObject"
      ],
      "Resource": "arn:aws:s3:::kj-test-bucket2/*"
    },
    {
      "Sid": "KishanIAM2DeleteGet",
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::349853822779:user/kishan-iam-2"
      },
      "Action": [
        "s3:DeleteObject",
        "s3:GetObject"
...
**Problem Encountered:** Bucket policy restrictions weren't working because `kishan-iam-2` had `AdministratorAccess` IAM policy attached.
...
**KMS CMK Setup:**
- **Key ARN:** `arn:aws:kms:us-east-1:349853822779:key/9e2992ba-130b-4f8e-b90b-4503f3f96719`
    ...
    --server-side-encryption-configuration '{
        "Rules": [{
            "ApplyServerSideEncryptionByDefault": {
                ...
                "KMSMasterKeyID": "arn:aws:kms:us-east-1:349853822779:key/9e2992ba-..."
            },
...
```bash
aws s3 cp dp.png s3://kj-test-bucket2/dp.png
# Error: AccessDenied - not authorized to perform: kms:GenerateDataKey
```

**Root Cause Analysis:**

Even though `kishan-iam` had S3 PutObject permission:
  ...
  "Statement": [
    {
      ...
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::349853822779:root"
      },
      "Action": "kms:*",
      "Resource": "*"
    },
    {
      "Sid": "AllowArgoCDDecrypt",
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::349853822779:role/dev8-secrets-role"
      },
      "Action": [
        "kms:Decrypt",
        "kms:DescribeKey"
    ...
    {
      "Sid": "kj-test-bucket2-encryption",
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::349853822779:user/kishan-iam"
      },
...
```json
{
  ...
  "Effect": "Allow",
  "Principal": {
    "AWS": "arn:aws:iam::349853822779:role/replication-role"
  },
  "Action": [
    "kms:Decrypt",
    "kms:DescribeKey"
...
aws configure --profile kishan-iam

# Use profile in commands
aws s3 ls --profile kishan-iam
```
...
```
┌─────────────────────────────────────────────────────────────┐
│                    AWS Account: 349853822779                 │
│                                                              │
│  ┌────────────────┐           ┌──────────────────────────┐  │
│  │  IAM Users     │           │   S3: kj-test-bucket2    │  │
│  │                │           │                          │  │
│  │ kishan-iam ────┼──────────►│  /public_space/*         │  │
│  │   (PutObject,  │           │    (Public Read)         │  │
│  │    GetObject)  │           │                          │  │
│  │                │           │  /other files            │  │
│  │ kishan-iam-2 ──┼──────────►│    (Private)             │  │
│  │   (DeleteObj,  │           │                          │  │
...
│           │    │  Key ID: 9e2992ba-130b-4f8e-...        │  │
│           │    │                                         │  │
│           └───►│  Permissions:                           │  │
│                │  - kishan-iam: Encrypt/Decrypt          │  │
│                │  - S3 Service: Encrypt/Decrypt          │  │
│                │  - dev8-secrets-role: Decrypt           │  │
│                └─────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
```
...
3. Add KMS Permissions to Key Policy
   {
     ...
     "Principal": {"AWS": "arn:aws:iam::...:user/kishan-iam"}
   }
   ↓
4. Retry S3 Upload
   aws s3 cp file.txt s3://bucket/
...
aws s3 cp local.txt s3://bucket/key
